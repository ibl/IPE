 ...
 // Module "store"
    angular.module('ipe:modules', []).controller("modulelist", ["$scope", "$http", function ($scope, $http) {
        ...
        $http.get("http://minervajs.org/lib").then(function (response) {
            ...
            $scope.updateLoadedModules();
        });
        $scope.loadScript = function (url) {
            IPE.loadScript(url, function (err) {
                if (err) {
                    IPE.ui.toast.error("Unable to load "+url);
                } else {
                    IPE.ui.toast.success("Successfully loaded "+url);
                    $scope.$apply($scope.updateLoadedModules);
                }
            });
        };
        $scope.updateLoadedModules = function () {
            IPE.modules( function (err, loadedModules) {
                $scope.modules.forEach( function (module) {
                    if (loadedModules.indexOf(module.url) >= 0 || loadedModules.indexOf(module.url+"/") >= 0) {
                        module.loaded = true;
                    }
                });
            });
